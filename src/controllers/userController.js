...
exports.signupUser = catchAsyncError(async (req, res, next) => {
  ...
  if (userExist) {
    return loginWithToken(userExist, "Login Successfully!!", 200, res);
  }
  ...
  const user = await User.create({
    ...
    avatar,
    userRole: "User",
    referCode: Math.floor(Math.random() * 10000000000),
    firebase,
  });
...
exports.updateMyPassword = catchAsyncError(async (req, res, next) => {
  ...
  if (user.firebase === true) {
    return next(
      new ErrorHandler("Please Update Password in google or facebook", 400)
    );
  }
